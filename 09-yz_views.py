...
# --------------------------------------------------
# Visualize one complete LiDAR scan in Y-Z
# --------------------------------------------------

scan_index = np.argmin(
    np.abs(lidar_positions - 5.0)
)
...
plt.xlabel("Y — Lateral distance from LiDAR (m)")
plt.ylabel("Z — Height (m)")
plt.title(
    f"LiDAR Vertical Scan at X = "
    f"{scan_results[scan_index]['x']:.2f} m"
)
