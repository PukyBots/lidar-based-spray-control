...
for nozzle_index, (z_min, z_max) in enumerate(nozzle_zones, start=1):
    ...
    for angle_deg in beam_angles_deg:
        ...
        # Horizontal beam: it stays at LiDAR height
        if abs(sin_angle) < 1e-9:
            if z_min <= LIDAR_HEIGHT_M <= z_max:
                usable_count += 1
            continue

        # Range at which beam reaches the lower and upper
        # boundaries of the nozzle zone
        r1 = (z_min - LIDAR_HEIGHT_M) / sin_angle
        r2 = (z_max - LIDAR_HEIGHT_M) / sin_angle

        r_near = min(r1, r2)
        r_far = max(r1, r2)

        # Does this beam intersect the nozzle zone
        # at a positive range within LiDAR range?
        if r_far >= 0 and r_near <= LIDAR_MAX_RANGE_M:

            usable_count += 1

    usable_beams[f"N{nozzle_index}"] = usable_count


print("\nGeometrically usable beams per nozzle zone:")

for nozzle, count in usable_beams.items():
    print(f"{nozzle}: {count} beams")