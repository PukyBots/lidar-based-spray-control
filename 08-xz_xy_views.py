...
# --------------------------------------------------
# Visualize LiDAR scan positions
# --------------------------------------------------
...
plt.scatter(
    lidar_positions,
    np.full(len(lidar_positions), LIDAR_HEIGHT_M),
    s=3,
    label="LiDAR path"
)
...
plt.title("Orchard Canopy and LiDAR Scanning Path")
...
plt.xlabel("Distance along row X (m)")
plt.ylabel("Height Z (m)")
plt.title("Simulated LiDAR Returns Along Orchard Row")
...
# --------------------------------------------------
# Orchard geometry: XY top view + XZ side view
# --------------------------------------------------
...
# Nozzle plane is 3 m behind LiDAR
nozzle_x = lidar_x - 3.0
...
ax.axvline(
    lidar_x,
    linestyle="--",
    label="LiDAR scan plane"
)

ax.set_xlabel("X — Travel direction (m)")
ax.set_ylabel("Y — Lateral direction (m)")
ax.set_title("Top View — LiDAR and Orchard Geometry")
...
ax.scatter(
    nozzle_x,
    LOWEST_NOZZLE_HEIGHT_M,
    s=100,
    marker="s",
    label="Nozzle plane"
)

# Draw six nozzle zones
for i, (z_min, z_max) in enumerate(nozzle_zones, start=1):
...
ax.set_title("Side View — Canopy Height and Nozzle Zones")
